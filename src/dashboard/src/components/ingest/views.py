...
def _es_results_to_appraisal_tab_format(record, record_map, directory_list, not_draggable=False):
    ...
    for node in directories:
        ...
        if parent is not None and not dir_record in parent['children']:
            parent['children'].append(dir_record)
            object_count = parent['object_count'] + 1
            parent['object_count'] = object_count
...
@decorators.elasticsearch_required()
def transfer_appraisal(request):
    """
    AJAX endpoint to query for and return transfer backlog items in the appraisal tab format.
    """
    # Get search parameters from request
    results = None
    queries, ops, fields, types = advanced_search.search_parameter_prep(request)
    ...
    # ]
    return_list = []
    # _es_results_to_appraisal_tab_format requires that paths MUST be sorted
    results.sort(key=lambda x: x['relative_path'])
    directory_map = {}
    for path in results:
        # If a path is in SIPArrange.original_path, then it shouldn't be draggable
