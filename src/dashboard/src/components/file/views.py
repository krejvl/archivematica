...
def transfer_file_tags(request, uuid):
    """
    ...
    Accepted methods and parameters:
        GET: requires a file UUID.
            Returns a JSON-encoded list of the file's tags on success.
        PUT: requires a file UUID, and document body must be a JSON-encoded list.
        ...
        DELETE: requiest a file UUID.
            Deletes all tags for the given file.
    """
    if request.method == 'GET':
        try:
            tags = elasticSearchFunctions.connect_and_get_file_tags(uuid)
        ...
        try:
            elasticSearchFunctions.connect_and_set_file_tags(uuid, tags)
        except elasticSearchFunctions.ElasticsearchError as e:
            response = {
                'success': False,
                'message': str(e),
        ...
        try:
            elasticSearchFunctions.connect_and_set_file_tags(uuid, [])
        except elasticSearchFunctions.ElasticsearchError as e:
            response = {
                'success': False,
                'message': str(e),
...
def bulk_extractor(request, uuid):
    """
    ...
    try:
        record = elasticSearchFunctions.get_transfer_file_info('fileuuid', uuid)
    except elasticSearchFunctions.ElasticsearchError as e:
        message = str(e)
        response = {
            'success': False,
    ...
    for report in reports:
        relative_path = os.path.join('logs', 'bulk-' + uuid, report + '.txt')
        url = storage_service.extract_file_url(f.transfer_id, relative_path)
        response = requests.get(url)

        if response.status_code != 200:
            message = 'Unable to retrieve ' + report + ' report for file with UUID ' + uuid
...
def file_details(request, uuid):
    try:
        source = elasticSearchFunctions.get_transfer_file_info('fileuuid', uuid)
    except elasticSearchFunctions.ElasticsearchError as e:
        message = str(e)
        response = {
            'success': False,
